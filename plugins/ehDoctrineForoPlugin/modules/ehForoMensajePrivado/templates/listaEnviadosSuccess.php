...
<?php $sf_response->setTitle('Mensajes privados enviados • '.ehForoConfig::getStatic('nombre')) // title ?>

<?php slot('eh_foro_navegador') ?>
<?php include_partial('ehForoTablon/navegador', array(
  'nav' => array(
    array('Mensajes enviados')
  ))); ?>
<?php end_slot() ?>

<div id="eh_foro_encabezado">
  <h1>Mensajes privados enviados</h1>
</div>

<div class="eh_foro_botones">
  <?php echo link_to(image_tag(ehForoConfig::getStatic('theme_path').'/images/btn-received.png', array('Mensajes recibidos')), '@eh_foro_privados_recibidos?pagina=1') ?>
</div>

<div class="eh_foro_opciones">
  <?php include_partial('ehForoMensajePrivado/paginas', array('pager' => $pager, 'uri' => '@eh_foro_privados_enviados')) ?>
</div>

<div id="eh_foro_privados_enviados" class="eh_foro_tabla eh_foro_round eh_foro_privados">
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="eh_foro_destinatario">Destinatario</th>
        <th class="eh_foro_fecha">Fecha envío</th>
      </tr>
    </thead>
    <tbody>
      <?php if($pager->getNbResults() < 1): ?>
      <tr>
        <td colspan="4">No tienes mensajes privados enviados.</td>
      </tr>
      <?php else: ?>
      ...
      <tr class="<?php echo $key % 2 == 0 ? 'par' : 'impar' ?>" id="<?php echo 'eh_foro_privado_'.$registro->getId() ?>">
        ...
        <td class="eh_foro_destinatario"><?php echo link_to($registro->getDestinatarioNick(), '@eh_foro_perfil?username='.$registro->getDestinatario()->getUsername()) ?></td>

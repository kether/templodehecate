...
class indiceComponents extends sfComponents
{
  ...
  public function executeMenuPrincipal(sfWebRequest $request)
  {
    ...
    foreach($menus as $key => $menu)
    {
      ...
      if($i == 1)
      {
        if($menusBD = Doctrine::getTable('tdhMenu')->getForPortada())
        {      
          foreach($menusBD as $menuBD)
          {
            $menuPrincipal[$menuBD->getSlug()] = array(
              'nombre'    => $menuBD->getNombre(),
              'uri'       => $menuBD->getEnlace() ? $menuBD->getEnlace() : '@homepage', 
              'foco'      => false,
              'especial'  => true);
          }
        }
  ...
  public function executeJuegosPopulares(sfWebRequest $request)
  {
    $this->populares  = Doctrine::getTable('tdhSeccion')->getPopularesActuales(array('limite' => 10));
  }
  
  public function executePortada(sfWebRequest $request)
  {
    Doctrine::getTable('tdhEnvioSocialPendiente')->enviarPendientes($this->getContext()->getConfiguration()->getEnvironment(), $this->getLogger());
    
    $this->destacados = Doctrine::getTable('tdhNoticia')->getAutorizados(3, array('filtro' => 'destacados'));
    $this->noticias   = Doctrine::getTable('tdhNoticia')->getAutorizados(4, array('filtro' => 'no-destacados'));
  }
  
  public function executeForo(sfWebRequest $request)
  {
    $this->foro       = Doctrine::getTable('ehForoHilo')->retrieveAutorizados();
    
    if($this->foro->count() < 1)
    {
      return sfView::NONE;
    }
  }
